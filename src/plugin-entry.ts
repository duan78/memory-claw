...
const plugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.logger.info(
      `memory-claw v2.4.49: Registered (db: ${dbPath}, model: ${embedding.model}, vectorDim: ${vectorDim}, CRITICAL FIX: dedup fixed (0.90→0.50 threshold, 3→50 candidates), locales: ${activeLocales.length})`
    );
